...
<form action="{{ route('support.kb.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-md-8">
            <div class="card-custom mb-4">
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control form-control-custom @error('title') is-invalid @enderror" value="{{ old('title') }}" required>
                    @error('title')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                
                <div class="mb-3">
                    <label class="form-label fw-bold">Isi Artikel</label>
                    <textarea name="content" class="form-control form-control-custom @error('content') is-invalid @enderror" rows="12" required>{{ old('content') }}</textarea>
            ...
            <div class="card-custom mb-4">
                <div class="mb-3">
                    ...
                    <select name="category" class="form-select form-control-custom @error('category') is-invalid @enderror" required>
                        ...
                        <option value="Account & Access" {{ old('category') == 'Account & Access' ? 'selected' : '' }}>Account & Access</option>
                    </select>
                ...
                <button type="submit" class="btn btn-primary-custom w-100">Simpan Artikel</button>
            </div>
        </div>
    </div>
</form>
